...
## Ambari TLS and Truststore
      ...
      if options.ssl
        ...
        throw Error "Required Property: options.truststore.password" if not options.truststore.password
        options.truststore.caname ?= 'hadoop_root_ca'
        options.truststore.type ?= 'jks'
        throw Error "Invalid Truststore Type: #{options.truststore.type}" unless options.truststore.type in ['jks', 'jceks', 'pkcs12']
...
`auth=KERBEROS;proxyuser=ambari`
      ...
      if options.jaas.enable
        ...
        throw Error "Require Property: jaas.realm or options.jaas.principal" unless options.jaas.realm
