...
```
    ...
    module.exports.push module.exports.configure = (ctx) ->
      ...
      # Layout
      hbase.conf_dir = hbase.conf_dir ?= '/etc/hbase/conf'
      hbase.log_dir = hbase.log_dir ?= '/var/log/hbase'
      ...
      hbase.site['hbase.master.keytab.file'] ?= '/etc/hbase/conf/hm.service.keytab' # was hm.service.keytab
      hbase.site['hbase.master.kerberos.principal'] ?= "hbase/_HOST@#{realm}"
      hbase.site['hbase.regionserver.keytab.file'] ?= '/etc/hbase/conf/rs.service.keytab' # was rs.service.keytab
      ...
      hbase.site['hbase.thrift.keytab.file'] ?= '/etc/hbase/conf/hbase.service.keytab'
      # TODO: add acl (http://hbase.apache.org/book.html#d3314e6371)
      hbase.site['hbase.thrift.kerberos.principal'] ?= "HTTP/_HOST@#{realm}" # Dont forget `grant 'thrift_server', 'RWCA'`
      hbase.site['hbase.rest.keytab.file'] ?= '/etc/hbase/conf/hbase.service.keytab'
      hbase.site['hbase.rest.kerberos.principal'] ?= "HTTP/_HOST@#{realm}" # Dont forget `grant 'rest_server', 'RWCA'`
      ...
      hbase.site['hbase.rest.authentication.kerberos.keytab'] ?= '/etc/hbase/conf/hbase.service.keytab'
      # Environment
      hbase.env ?=  {}
...
Instructions to [install the HBase RPMs](http://docs.hortonworks.com/HDPDocuments/HDP1/HDP-1.3.2/bk_installing_manually_book/content/rpm-chap9-1.html)
    ...
    module.exports.push name: 'HBase # Layout', timeout: -1, handler: (ctx, next) ->
      {hadoop_group, hbase} = ctx.config.ryba
      ctx.mkdir [
    ...
    module.exports.push name: 'HBase # Configure', handler: (ctx, next) ->
      {hbase} = ctx.config.ryba
