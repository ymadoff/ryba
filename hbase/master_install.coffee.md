...
Environment passed to the Master before it starts.   
    ...
    module.exports.push name: 'HDFS NN # Opts', handler: (ctx, next) ->
      ...
      console.log hbase.master_opts
      ctx.write
        destination: "#{hbase.conf_dir}/hbase-env.sh"
        match: /^export HBASE_MASTER_OPTS="(.*) \$\{HBASE_MASTER_OPTS\}" # GENERATED BY RYBA, DONT OVEWRITE/mg
        replace: "export HBASE_MASTER_OPTS=\"${HBASE_MASTER_OPTS} #{hbase.master_opts}\" # GENERATED BY RYBA, DONT OVEWRITE"
        append: /^export HBASE_MASTER_OPTS=".*"$/mg
        backup: true
      , next
