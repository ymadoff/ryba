...
#wdavidw: comment next line to inject hadoop options
#export HADOOP_OPTS="-Djava.net.preferIPv4Stack=true ${HADOOP_OPTS}"
<%- @config.hdp.hadoop_opts %>

# Command specific options appended to HADOOP_OPTS when specified
export HADOOP_NAMENODE_OPTS="-server -XX:ParallelGCThreads=8 -XX:+UseConcMarkSweepGC -XX:ErrorFile=<%- @config.hdp.hdfs_log_dir %>/$USER/hs_err_pid%p.log -XX:NewSize=200m -XX:MaxNewSize=640m -Xloggc:<%- @config.hdp.hdfs_log_dir %>/$USER/gc.log-`date +'%Y%m%d%H%M'` -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps -Xms1024m -Xmx1024m -Dhadoop.security.logger=INFO,DRFAS -Dhdfs.audit.logger=INFO,DRFAAUDIT ${HADOOP_NAMENODE_OPTS}"
HADOOP_JOBTRACKER_OPTS="-server -XX:ParallelGCThreads=8 -XX:+UseConcMarkSweepGC -XX:ErrorFile=<%- @config.hdp.hdfs_log_dir %>/$USER/hs_err_pid%p.log -XX:NewSize=200m -XX:MaxNewSize=200m -Xloggc:<%- @config.hdp.hdfs_log_dir %>/$USER/gc.log-`date +'%Y%m%d%H%M'` -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps -Xmx1024m -Dhadoop.security.logger=INFO,DRFAS -Dmapred.audit.logger=INFO,MRAUDIT -Dhadoop.mapreduce.jobsummary.logger=INFO,JSA ${HADOOP_JOBTRACKER_OPTS}"
...
export HADOOP_LOG_DIR=<%- @config.hdp.hdfs_log_dir %>/$USER

# History server logs
export HADOOP_MAPRED_LOG_DIR=<%- @config.hdp.mapred_log_dir %>/$USER

# Where log files are stored in the secure data environment.
export HADOOP_SECURE_DN_LOG_DIR=<%- @config.hdp.hdfs_log_dir %>/$HADOOP_SECURE_DN_USER
...
# wdavidw, default was /var/run/hadoop/$USER
export HADOOP_PID_DIR=/var/run/hadoop-hdfs/$USER
export HADOOP_SECURE_DN_PID_DIR=/var/run/hadoop-hdfs/$HADOOP_SECURE_DN_USER
